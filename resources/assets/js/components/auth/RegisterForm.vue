<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
   ...
   <div class="col-xs-7">
    <label>
     <div class="checkbox_register">
      <label>
       <input type="checkbox" name="terms" checked="checked" v-model="form.terms" :class="{ 'has-error': form.errors.has('terms') }">
       <a href="#" data-toggle="modal" data-target="#termsModal" :class="{ 'text-danger': form.errors.has('terms') }" v-text="trans('adminlte_lang_message.conditions')" style="font-size:12px"></a>
      </label>
     </div>
    </label>
   </div>
  ...
  <div v-if="form.errors.has('terms')" class="form-group has-feedback has-error">
